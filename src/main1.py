import time
import network
import socket
import ujson
import math

from m5stack import lcd

imu_obj = None
try:
    from m5stack import imu
    imu_obj = imu.IMU()
except:
    try:
        from imu import IMU
        imu_obj = IMU()
    except:
        imu_obj = None

# ------------- CONFIG -------------
WIFI_SSID = "GTDM"
WIFI_PASS = "12345678"

SLAVE_RX_PORT  = 5006   # esclavo recibe estado aquí
MASTER_RX_PORT = 5005   # maestro recibe input aquí (esclavo envía a este puerto)

MASTER_IP = "192.168.0.164"  

W, H = 320, 240
PADDLE_W = 6
PADDLE_H = 40
BALL_SZ  = 6

LPF = 0.85
K   = 0.90
DEADBAND = 1.2

SEND_EVERY_MS = 25  # ~40 Hz

def clamp(v, lo, hi):
    return lo if v < lo else (hi if v > hi else v)

def wifi_connect():
    wlan = network.WLAN(network.STA_IF)
    wlan.active(True)
    if not wlan.isconnected():
        wlan.connect(WIFI_SSID, WIFI_PASS)
        t0 = time.ticks_ms()
        while not wlan.isconnected():
            time.sleep_ms(150)
            if time.ticks_diff(time.ticks_ms(), t0) > 15000:
                raise RuntimeError("No conecta a WiFi (timeout)")
    return wlan.ifconfig()[0]

def imu_pitch_deg():
    if imu_obj is None:
        return 0.0
    ax = ay = az = 0.0
    try:
        ax, ay, az = imu_obj.getAccelData()
    except:
        try:
            a = imu_obj.acceleration
            ax, ay, az = a[0], a[1], a[2]
        except:
            try:
                ax, ay, az = imu_obj.acceleration()
            except:
                return 0.0
    return math.atan2(ax, math.sqrt(ay*ay + az*az)) * 57.2958

# ------------- GAME STATE (lo pinta el esclavo) -------------
p1_y = (H - PADDLE_H)/2
p2_y = (H - PADDLE_H)/2
ball_x = W/2
ball_y = H/2
s1 = 0
s2 = 0

pitch0 = 0.0
pitch_f = 0.0

prev_p1y = prev_p2y = prev_bx = prev_by = -1

def draw_scores():
    lcd.rect(0, 0, W, 22, lcd.BLACK, lcd.BLACK)
    lcd.text(10, 4, "P1:%d" % s1, lcd.WHITE)
    lcd.text(W-90, 4, "P2:%d" % s2, lcd.WHITE)

def draw_net_and_scores():
    lcd.clear(lcd.BLACK)
    y = 0
    while y < H:
        lcd.line(W//2, y, W//2, y+4, lcd.DARKGREY)
        y += 10
    draw_scores()

def draw_paddle(x, y, color):
    lcd.rect(x, int(y), PADDLE_W, PADDLE_H, color, color)

def draw_ball(x, y, color):
    lcd.rect(int(x), int(y), BALL_SZ, BALL_SZ, color, color)

def render():
    global prev_p1y, prev_p2y, prev_bx, prev_by

    if prev_p1y >= 0: draw_paddle(0, prev_p1y, lcd.BLACK)
    if prev_p2y >= 0: draw_paddle(W - PADDLE_W, prev_p2y, lcd.BLACK)
    if prev_bx  >= 0: draw_ball(prev_bx, prev_by, lcd.BLACK)

    draw_paddle(0, p1_y, lcd.WHITE)
    draw_paddle(W - PADDLE_W, p2_y, lcd.WHITE)
    draw_ball(ball_x, ball_y, lcd.WHITE)

    prev_p1y = int(p1_y)
    prev_p2y = int(p2_y)
    prev_bx  = int(ball_x)
    prev_by  = int(ball_y)

    draw_scores()

def calibrate_imu():
    global pitch0, pitch_f
    lcd.rect(0, 100, W, 20, lcd.BLACK, lcd.BLACK)
    lcd.text(10, 104, "Calibrando IMU... quieto", lcd.WHITE)
    s = 0.0
    N = 200
    for _ in range(N):
        s += imu_pitch_deg()
        time.sleep_ms(10)
    pitch0 = s / N
    pitch_f = 0.0

def update_p2_from_imu():
    global p2_y, pitch_f
    pitch = imu_pitch_deg() - pitch0
    pitch_f = LPF * pitch_f + (1.0 - LPF) * pitch
    if abs(pitch_f) < DEADBAND:
        pitch_f = 0.0
    p2_y += pitch_f * K
    p2_y = clamp(p2_y, 0, H - PADDLE_H)

# ------------- UDP -------------
sock = None
master_addr = None
last_seq = -1

def udp_setup():
    global sock, master_addr
    sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    sock.bind(("0.0.0.0", SLAVE_RX_PORT))
    sock.setblocking(False)
    master_addr = (MASTER_IP, MASTER_RX_PORT)

def send_input():
    msg = {"t": "inp", "p2": int(p2_y)}
    try:
        sock.sendto(ujson.dumps(msg), master_addr)
    except:
        pass

def recv_state():
    global p1_y, p2_y, ball_x, ball_y, s1, s2, last_seq
    try:
        data, addr = sock.recvfrom(512)
    except:
        return

    try:
        msg = ujson.loads(data)
    except:
        return
    if msg.get("t") != "st":
        return

    seq = msg.get("seq", 0)
    if seq <= last_seq:
        return
    last_seq = seq

    p1_y = float(msg.get("p1", p1_y))
    # OJO: p2_y local la controla IMU, pero el maestro también lo manda.
    # Para evitar “tirones”, nos quedamos con el local.
    s1 = int(msg.get("s1", s1))
    s2 = int(msg.get("s2", s2))

    b = msg.get("ball", {})
    ball_x = float(b.get("x", ball_x))
    ball_y = float(b.get("y", ball_y))

# ------------- MAIN -------------
lcd.clear(lcd.BLACK)
lcd.text(10, 20, "PONG ESCLAVO (MicroPython)", lcd.WHITE)

ip = wifi_connect()
lcd.text(10, 50, "IP: %s" % ip, lcd.WHITE)
lcd.text(10, 70, "RX:%d  TX->%s:%d" % (SLAVE_RX_PORT, MASTER_IP, MASTER_RX_PORT), lcd.WHITE)

calibrate_imu()
draw_net_and_scores()

udp_setup()

last_send = time.ticks_ms()

while True:
    now = time.ticks_ms()

    update_p2_from_imu()
    recv_state()
    render()

    if time.ticks_diff(now, last_send) >= SEND_EVERY_MS:
        send_input()
        last_send = now

    time.sleep_ms(5)
